...
# Master and log directory was identificatied explicit
spark = SparkSession\
            .builder\
            .master('spark://0a7a8fe5c3d8:7077')\
            .appName('spark_basic')\
...
df_people = spark.read.option('header', 'true').schema(people_schema).csv('/opt/bitnami/spark/work/top_100_richest.csv')

df_country = spark.read.option('header', 'true').schema(country_schema).csv('/opt/bitnami/spark/work/wiki_number_of_billionaires.csv')
...
df_parquet = spark.read.schema(people_schema).parquet('/opt/bitnami/spark/work/data_parquet')

df_parquet_filtered = df_parquet.filter(col('nationality') == 'Russia')

df_last = df_last.union(df_parquet_filtered)

df_last.write.option("header", 'true').mode('overwrite').csv('/opt/bitnami/spark/work/1_p3.csv')
...
with open('/opt/bitnami/spark/work/plans.txt', 'w') as file:
    file.write(plans)
    
#я оставил оба варианта вывода планов, но второй который plans_1 предпочтительнее. Планы одинаковые

with open('/opt/bitnami/spark/work/plans_1.txt', 'w') as file:
    file.write(plans_1)
